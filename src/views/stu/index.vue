...
const deleteById=async(id)=>{
  ElMessageBox.confirm('您确认删除此数据吗?' , '删除学生', {confirmButtonText:'确认', cancelButtonText:'取消', type:'warning'})
    .then(async () => {
      let result =  await deleteByIdsApi(`${id}`)
      if(result.code) {
...
const deleteByIds=async()=>{ 
  if(selectIds.value.length===0){
    ElMessage.info('请选择要删除的记录')
    return
  }
  ElMessageBox.confirm('您确认删除此数据吗?' , '删除学生', {confirmButtonText:'确认', cancelButtonText:'取消',type:'warning'})
    .then(async () => {
      let result =  await deleteByIdsApi(selectIds.value.join(','))
      if(result.code) {
        ElMessage.success('删除成功')
        queryPage()
      }else {
  ...
  <el-form :inline="true" :model="searchStu" class="demo-form-inline">
    ...
    <el-form-item label="最高学历">
      <el-select v-model="searchStu.degree" placeholder="请选择">
        <el-option v-for="degree in degreeList" :key="degree.value" :label="degree.name" :value="degree.value" />
      </el-select>
    </el-form-item>
  ...
  <el-dialog v-model="dialogVisible" :title="dialogTitle" width="50%">
        ...
        <el-col :span="12">
          <el-form-item label="性别" :label-width="labelWidth"  prop="gender">
            <el-select v-model="stu.gender" placeholder="请选择" style="width: 100%;">
              <el-option v-for="gender in genderList" :key="gender.value" :label="gender.name" :value="gender.value" />
            </el-select>
          </el-form-item>
        </el-col>
        <el-col :span="12">
        ...
        <el-col :span="12">
          <el-form-item label="是否院校" :label-width="labelWidth" prop="isCollege">
            <el-select v-model="stu.isCollege" placeholder="请选择" style="width: 100%;">
              <el-option v-for="w in whetherList" :key="w.value" :label="w.name" :value="w.value" />
            </el-select>
          </el-form-item>
        </el-col>
      </el-row>
    ...
    <template #footer>
      <span class="dialog-footer">
        <el-button @click="dialogVisible = false; resetForm(stuFormRef)">取消</el-button>
